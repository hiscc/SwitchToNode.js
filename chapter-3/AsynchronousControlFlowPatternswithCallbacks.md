...
正如我们所知，当任务请求一个新的异步操作时任务的控制权会交还给事件轮询然后事件轮询启动另一个任务。一个适当形容这种流程的词就是并发性，但是我们为了简单还是用并行吧。
...
竞争条件可以引发很多问题，即使是在单线程环境中。在很多情况下它们会导致数据出错而且难以调试因为这问题非常短暂，所以最佳实践是在运行并行任务时多次检查。
...
下一章将介绍更多的高级技术例如 Promise 和 Generator。当你掌握了这些技术就可以选择最佳的解决方案了。
